...
def training(model, simialrity_score_calcualtor, batch_provider, batch_size, max_iterations, optimizer, device):
    ...
    for iter_i in range(max_iterations):
        ...
        batch_ref_patches, batch_pos_patches, batch_neg_patches = next(batch_provider.iterate_batches(batch_size=batch_size))
        ...
        if iter_i % 100 == 0:
            ...
            print('Average loss in last 100 iterations:           {}'.format(sum(losses[-100:])/100))
            print('Average train accuracy in last 100 iterations: {}'.format(sum(train_accuracies[-100:])/100))
...
if __name__ == '__main__':
    ...
    hyper_param = {'path_size': (9, 9), 'max_iteration': 3, 'batch_size': 128, 'learning_rate': 1e-4}
    ...
    patch_provider_obj = PatchProvider(data=dataset_train, patch_size=hyper_param['path_size'], N=(4, 10), P=1)
    ...
    training(model=model, 
             ...
             batch_provider=patch_provider_obj,
